...
@Injectable()
export class NylasCalendarProvider implements ICalendarProvider {
  constructor(private readonly grantId: string) {}

  async createEvent(event: CalendarEvent): Promise<{ eventId: string; event: CalendarEvent }> {
    throw new Error('Nylas Calendar provider not yet implemented');
  }
